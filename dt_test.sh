...
TXRATE="$2";
[ -z "$TXRATE" ] && echo "The third parametet must be number of txRate option (like 500)" && exit;

DOMAIN="$3";
[ -z "$DOMAIN" ] && echo "The second parameter must be DOMAIN name (like \"uk.db.com\") !" && exit;
...
DT_RXTX_OPT="-txDurationMs 30000 -txRate $TXRATE -rxBufferSize 250 -packetSize 8192";
